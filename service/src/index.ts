...
import { handleChatCompletions, handleModelsList, handleResponses } from "./openai-gateway"
import {
  handleClewdrList, handleClewdrDetail,
  handleOaiList, handleOaiDetail,
  handleStats, handleDistincts,
  handleGetSettings, handleUpdateSettings, handlePurge,
} from "./audit-api"
...
app.use(express.json({ limit: process.env.BODY_SIZE_LIMIT || '2mb' }))

// ── Global error safety net ──
// Anything uncaught elsewhere (bad promise chain inside a DAO helper, audit
// serialization hiccup, etc.) would otherwise crash the worker and drop every
// in-flight stream. Log and keep running.
...
router.post('/openai/v1/responses', express.json({ limit: '50mb' }), gatewayAuth, handleResponses)
router.post('/openai/responses', express.json({ limit: '50mb' }), gatewayAuth, handleResponses)
...
router.get('/clewdr/cookies', auth, async (req, res) => {
  try {
    const refresh = req.query.refresh === 'true'
    const data = await clewdrFetch(`/api/cookies${refresh ? '?refresh=true' : ''}`)
...
const KIRO_GW_ADMIN_KEY = process.env.KIRO_GW_ADMIN_KEY || KIRO_GW_KEY

async function kiroGwFetch(path: string, options: any = {}): Promise<any> {
  const url = `${KIRO_GW_URL}${path}`
  // Admin endpoints (/v1/pool/*) need admin key; others need chat key.
  const key = path.startsWith('/v1/pool') ? KIRO_GW_ADMIN_KEY : KIRO_GW_KEY
  const res = await fetch(url, {
    ...options,
    headers: {
      'Content-Type': 'application/json',
...
router.get('/kiro/usage', auth, async (req, res) => {
  try {
    const qs = req.query.refresh ? '?refresh=1' : ''
    const data = await kiroGwFetch(`/v1/pool/usage${qs}`)
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})

router.post('/kiro/refresh-quotas', auth, async (_req, res) => {
  try {
    const data = await kiroGwFetch('/v1/pool/refresh-quotas', { method: 'POST' })
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})

router.get('/kiro/history', auth, async (req, res) => {
  try {
    const hours = req.query.hours ? `?hours=${encodeURIComponent(String(req.query.hours))}` : ''
    const accountId = req.query.account_id ? `${hours ? '&' : '?'}account_id=${encodeURIComponent(String(req.query.account_id))}` : ''
    const bucket = req.query.bucket ? `${(hours || accountId) ? '&' : '?'}bucket=${encodeURIComponent(String(req.query.bucket))}` : ''
    const data = await kiroGwFetch(`/v1/pool/history${hours}${accountId}${bucket}`)
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})
...
router.post('/kiro/accounts/upload', auth, async (req, res) => {
  try {
    ...
    const r = await fetch(`${KIRO_GW_URL}/v1/pool/accounts/upload`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${KIRO_GW_ADMIN_KEY}`,
...
router.delete('/kiro/accounts/:id', auth, async (req, res) => {
  try {
    const r = await fetch(`${KIRO_GW_URL}/v1/pool/accounts/${req.params.id}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${KIRO_GW_ADMIN_KEY}` },
    })
    if (!r.ok) throw new Error(`KiroGW ${r.status}: ${(await r.text()).slice(0, 200)}`)
    res.json({ status: 'Success' })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
...
router.patch('/kiro/accounts/:id/proxy', auth, async (req, res) => {
  try {
    const r = await fetch(`${KIRO_GW_URL}/v1/pool/accounts/${req.params.id}/proxy`, {
      method: 'PATCH',
      headers: { 'Authorization': `Bearer ${KIRO_GW_ADMIN_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ proxy_url: req.body.proxy_url ?? null }),
    })
    if (!r.ok) throw new Error(`KiroGW ${r.status}: ${(await r.text()).slice(0, 200)}`)
...
// ─── Kiro Account Login (via login server) ─────────────────────────

router.post('/kiro/login/start', auth, async (req, res) => {
  try {
    const label = req.body?.label || ''
    const qs = label ? `?label=${encodeURIComponent(label)}` : ''
    const data = await kiroGwFetch(`/v1/pool/login/start${qs}`, { method: 'POST' })
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})

router.get('/kiro/login/list', auth, async (_req, res) => {
  try {
    const data = await kiroGwFetch('/v1/pool/login')
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Success', data: { sessions: [] } }) }
})

router.get('/kiro/login/:label', auth, async (req, res) => {
  try {
    const data = await kiroGwFetch(`/v1/pool/login/${encodeURIComponent(req.params.label)}`)
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})

router.post('/kiro/login/:label/complete', auth, async (req, res) => {
  try {
    const data = await kiroGwFetch(`/v1/pool/login/${encodeURIComponent(req.params.label)}/complete`, { method: 'POST' })
    res.json({ status: 'Success', data })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})

router.delete('/kiro/login/:label', auth, async (req, res) => {
  try {
    await kiroGwFetch(`/v1/pool/login/${encodeURIComponent(req.params.label)}`, { method: 'DELETE' })
    res.json({ status: 'Success' })
  } catch (e: any) { res.json({ status: 'Fail', message: e.message }) }
})
...
// ─── Audit Log API (admin-v2 查看面板) ───
router.get("/audit/clewdr/logs", auth, handleClewdrList)
router.get("/audit/clewdr/logs/:id", auth, handleClewdrDetail)
router.get("/audit/oai/logs", auth, handleOaiList)
router.get("/audit/oai/logs/:id", auth, handleOaiDetail)
router.get("/audit/stats", auth, handleStats)
router.get("/audit/distincts", auth, handleDistincts)
router.get("/audit/settings", auth, handleGetSettings)
router.post("/audit/settings", auth, handleUpdateSettings)
router.post("/audit/purge", auth, handlePurge)
