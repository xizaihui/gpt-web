        ...
        if self.stream {
            ...
            let stream = try_stream! {
                ...
                while let Some(event) = stream.try_next().await? {
                    ...
                    // Split large content_block_delta into per-char events for smooth streaming
                    let mut split = false;
                    if let Ok(evt) = serde_json::from_str::<StreamEvt>(&event.data) {
                        if evt.r#type == "content_block_delta" {
                            if let Some(delta) = &evt.delta {
                                let delta_type = delta.get("type").and_then(|v| v.as_str()).unwrap_or("");
                                let text_field = if delta_type == "text_delta" { "text" }
                                    else if delta_type == "thinking_delta" { "thinking" }
                                    else { "" };
                                if !text_field.is_empty() {
                                    if let Some(text) = delta.get(text_field).and_then(|v| v.as_str()) {
                                        if text.len() > 1 {
                                            split = true;
                                            ...
                                            for ch in text.chars() {
                                                let chunk = serde_json::json!({
                                                    ...
                                                    "index": idx,
                                                    "delta": { "type": delta_type, text_field: ch.to_string() }
                                                });
                ...
                if !acc.is_empty() {
                    ...
                    let out = out.unwrap_or_else(|| {
                        let usage = crate::types::claude::Usage { input_tokens: input_tokens as u32, output_tokens: 0, cache_creation_input_tokens: 0, cache_read_input_tokens: 0 };
