...
impl ClaudeCodeState {
    ...
    fn merge_anthropic_beta_header(extra: Option<&str>, use_context_1m: bool) -> String {
        // Whitelist of beta flags known to work with Claude's OAuth API (claude.ai backend).
        // Unknown/unsupported flags are silently dropped to prevent 400 errors.
        ...
        fn is_allowed(token: &str) -> bool {
            ...
            ALLOWED_PREFIXES.iter().any(|prefix| lower.starts_with(prefix))
        }
        ...
        let mut push = |token: &str| {
            ...
            if !is_allowed(trimmed) {
                tracing::debug!("Filtered unsupported beta flag: {}", trimmed);
                return;
            }
            if seen.insert(key) {
