...
use crate::{
    claude_web_state::{ClaudeWebState, BOOTSTRAP_CACHE},
...
impl ClaudeWebState {
    /// Bootstraps the application state, using cache when available.
    /// Cache key is the cookie string — same cookie → same org_uuid & capabilities.
    pub async fn bootstrap(&mut self) -> Result<(), ClewdrError> {
        // Check cache first
        let cookie_key = self.cookie_cache_key();
        if let Some(cached) = BOOTSTRAP_CACHE.get(&cookie_key) {
            self.org_uuid = Some(cached.org_uuid.clone());
            self.capabilities = cached.capabilities.clone();
            info!(
                "[BOOTSTRAP-CACHE] hit for cookie {}… org={}",
                &cookie_key[..12.min(cookie_key.len())],
                cached.org_uuid
            );
            return Ok(());
        }

        // Cache miss — do full bootstrap
        info!("[BOOTSTRAP] cache miss, doing full bootstrap");
        self.bootstrap_full().await?;

        // Store in cache
        if let Some(ref org_uuid) = self.org_uuid {
            BOOTSTRAP_CACHE.insert(
                cookie_key,
                super::BootstrapCacheEntry {
    ...
    /// Full bootstrap — original logic, called on cache miss
    async fn bootstrap_full(&mut self) -> Result<(), ClewdrError> {
        let end_point = self
            .endpoint
            .join("api/bootstrap")
